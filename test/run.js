...
(async () => {
  ...
  test("PDF: оформление по ГОСТ — основная надпись, графы подшивки, спецификация", () => {
    ...
    ok(/Формат А4/.test(html), "формат листа под рамкой — кириллицей, как в ГОСТ 2.301");
  ...
  test("PDF: комплект ЭОМ — марка, общие данные, расчётные нагрузки, кабельный журнал", () => {
    const p = EP.Plan.Core.createProject("эом");
    p.address = "г. Москва, ул. Тестовая, д. 1";
    p.docCode = "074/1/26";            // марку в шифр руками НЕ вписываем — её приписывает марка
    p.settings.mainBreaker = 32; p.settings.mainRcd = true;
    const r = M.newRoom(G.rectPoints(0, 0, 500, 400), "Кухня");
    p.rooms.push(r);
    const c1 = M.newCircuit("QF1", "#ef4444", 16); c1.rcd = true;
    const c2 = M.newCircuit("QF2", "#22c55e", 25); c2.rcd = true;
    p.circuits.push(c1, c2);
    p.panels.push(M.newPanel(250, 20, "Щит квартирный"));
    const so = (off, c) => { const e = M.newElement("socket", r.id + ":0", off, 30, "power"); e.circuitId = c.id; p.elements.push(e); };
    so(80, c1); so(160, c1);
    // СМЕШАННАЯ линия: розетка идёт ПЕРВОЙ, кондиционер вторым — cos φ обязан считаться
    // по ВСЕМ элементам линии, а не по первому (иначе двигательная нагрузка терялась)
    so(240, c2);
    const ac = M.newElement("ac", r.id + ":2", 100, 230, "ac"); ac.circuitId = c2.id; p.elements.push(ac);
    EP.Plan.Core.commit(); EP.Plan.Core.persist("seed");
    EP.Plan.Routes.build({ silent: true });
    const html = EP.Plan.Export.sheetHtml(p);
    // ---- марка основного комплекта ----
    ok(/074\/1\/26-ЭОМ/.test(html), "марка приписана к шифру документа");
    ok(/Электрооборудование силовое/.test(html), "наименование раздела в штампе — по марке");
    p.mark = "СС";
    ok(/Системы связи/.test(EP.Plan.Export.sheetHtml(p)), "смена марки меняет наименование раздела");
    p.mark = "ЭОМ"; p.docCode = "074/1/26-ЭОМС";
    ok(EP.Plan.Export.sheetHtml(p).indexOf("ЭОМС-ЭОМ") < 0, "марка, уже вписанная в шифр, не приписывается второй раз");
    p.docCode = "074/1/26";
    // ---- лист «Общие данные» (ГОСТ Р 21.101) ----
    ok(/Ведомость рабочих чертежей основного комплекта/.test(html), "ведомость рабочих чертежей");
    // ИМЕННО по классу таблицы: строка «gd-ved» встречается и в <style> листа (он идёт
    // раньше самих листов), и первый split дал бы кусок CSS вместо ведомости
    const ved = (html.split('class="tb gd-ved"')[1] || "").split("</table>")[0];
    ok(/Титульный лист/.test(ved) && /Общие данные/.test(ved), "ведомость перечисляет и титул, и сами «Общие данные»");
    eq((ved.match(/<tr><td class="st-c">/g) || []).length, (html.match(/class="sheet"/g) || []).length,
      "строк в ведомости столько же, сколько листов в альбоме");
    ok(/ГОСТ Р 21\.101-2020/.test(html) && /ПУЭ, 7-е издание/.test(html), "ведомость ссылочных и прилагаемых документов");
    ok(/Общие указания/.test(html), "общие указания");
    ok(/вводной аппарат 32 А/.test(html), "указания собраны из НАСТРОЕК проекта, а не заглушка");
    ok(/линий с УЗО: 2/.test(html), "…включая реальное число линий с УЗО");
    ok(!/tp-ved/.test(html), "на титуле ведомости больше нет (она налезала на основную надпись)");
    // ---- таблица расчётных нагрузок ----
    ok(/Таблица расчётных нагрузок/.test(html), "лист расчётных нагрузок");
    ok(/>0,20</.test(html), "Ру линии из двух розеток — по удельной мощности точек");
    ok(/>1,60</.test(html), "Ру смешанной линии (розетка + кондиционер)");
    ok(/>0,85</.test(html), "cos φ 0,85 на линии с кондиционером, хотя первой в ней стоит розетка");
    ok(/коэффициентом одновременности/.test(html), "итог по вводу");
    // ---- кабельный журнал ----
    ok(/Кабельный журнал/.test(html), "лист кабельного журнала");
    ok(/Щит квартирный/.test(html), "начало линии — щит");
    // заголовок листа в разметке обычным регистром (в КАПС его переводит CSS); берём
    // именно <h2> и режем до штампа этого же листа
    const li = html.indexOf("<h2>Кабельный журнал</h2>");   // именно заголовок листа: то же название есть и в ведомости, и в штампе
    const log = li >= 0 ? html.slice(li, html.indexOf('class="stamp"', li)) : "";
    ok(/Кухня/.test(log), "конец линии — помещение");
    ok(/ВВГнг/.test(log), "марка кабеля — та же, что в однолинейке и спецификации");
    // развёртки и однолинейка ограничены ПОЛЕМ ЧЕРТЕЖА, а не высотой листа: с максимумами
    // от листа они налезали на основную надпись (замер живым прогоном: 41 и 77 мм захода)
    const src = require("fs").readFileSync(require("path").join(__dirname, "..", "assets", "js", "modules", "plan", "plan-export.js"), "utf8");
    ok(/--areaH: \$\{planArea\(\)\.h\}mm/.test(src), "высота поля чертежа объявлена переменной листа");
    ok(/\.unf \{[^}]*max-height: calc\(var\(--areaH\)/.test(src), "развёртка не выше поля чертежа");
    ok(/\.schemebox svg \{[^}]*max-height: calc\(var\(--areaH\)/.test(src), "однолинейка не выше поля чертежа");
    ok(!/max-height: \$\{pg\(\)\.h - 65\}mm/.test(src), "максимума от высоты ЛИСТА больше нет");
  });
  test("PDF: «Общие данные» знают свой объём — ведомость перечисляет и сами себя", () => {
    // на длинном альбоме ведомость не влезает на один лист, и «Общие данные» становятся
    // двумя листами — которые сами обязаны попасть в ведомость (иначе «Листов N» в штампе
    // разойдётся с реальным числом листов). Проверяем сходимость напрямую: на коротком
    // тестовом проекте поломку этой арифметики не видно ни по одному другому признаку.
    const f = EP.Plan.Export.genSheetCount;
    eq(f(27), 1, "27 листов + ведомость на один лист");
    eq(f(28), 2, "28 листов уже требуют второго листа ведомости — и он тоже в неё попадает");
    eq(f(54), 2, "54 + 2 = 56 строк ложатся ровно в два листа");
    eq(f(55), 3, "…а 55 требуют третьего");
  });
  test("PDF: марка комплекта — дефолт и бэкофилл старого проекта", () => {
    const p = EP.Plan.Core.createProject("mk");
    eq(p.mark, "ЭОМ", "у нового проекта марка ЭОМ");
    const raw = JSON.parse(EP.Plan.Core.exportJSON());
    delete raw.mark;
    const p2 = EP.Plan.Core.importJSON(JSON.stringify(raw));
    eq(p2.mark, "ЭОМ", "старый проект без марки получает её бэкофиллом");
  });
  test("PDF: масштаб чертежа подбирается плотнее и формат листа идёт в штамп", () => {
    ...
    ok(/Формат А4/.test(html), "формат листа подписан под рамкой");
  ...
  test("смета: экспорт даёт конверт ep-estimate со всеми позициями", () => {
    ...
    ok(data.type === "ep-estimate" && data.v === 1, "тип и версия конверта");
    ok(data.name === "кв.390" && typeof data.exportedAt === "string", "имя и дата в конверте");
    eq(data.items.length, 2, "позиции выгружены");
    const w = data.items.find((x) => x.type === "work");
  ...
  test("смета: импорт читает конверт, голый массив и запятую в числах", () => {
    ...
    ok(arr && arr.items[0].type === "material" && arr.items[0].qty === 500, "голый массив и русский тип");
    const comma = E.parseImport(JSON.stringify([{ type: "work", name: "Штроба", unit: "м", qty: "17,8" }]));
    ok(comma && comma.items[0].qty === 17.8, "количество с запятой");
    ok(E.parseImport("не json") === null && E.parseImport(JSON.stringify({ foo: 1 })) === null, "мусор отклонён");
  ...
  test("смета: импорт «добавить» складывает одинаковые, «заменить» очищает", () => {
    ...
    const file = JSON.stringify({ type: "ep-estimate", items: [
      ...
      { type: "work", name: "Прокладка кабеля", unit: "м", qty: 1220 }
    ] });
    E.importJSON(file, "add");
    let items = E.getItems();
    eq(items.length, 2, "одинаковая позиция не продублировалась");
    eq(items.find((x) => x.name === "Стяжки 5х350").qty, 600, "количество сложилось");
    E.importJSON(file, "replace");
    items = E.getItems();
  ...
  test("смета по работам: печатный бланк с этапами, часами и сроком", () => {
    ...
    ok(/3 чел\. × 8 ч\/день/.test(html), "бригада печатается в документе");
  ...
  test("смета по работам: рабочий день интервалами, дни и общие часы", () => {
    const W = loadEstimate().EstimateWorks;
    eq(W.shiftHours([{ from: "10:00", to: "13:00" }, { from: "15:00", to: "18:00" }]), 6,
      "10–13 и 15–18 это 6 рабочих часов, а не 8");
    eq(W.shiftText([{ from: "10:00", to: "13:00" }, { from: "15:00", to: "18:00" }]), "10:00–13:00, 15:00–18:00",
      "режим дня печатается человекочитаемо");
    // битые интервалы не учитываются вовсе, а не дают отрицательные часы
    eq(W.shiftHours([{ from: "18:00", to: "10:00" }]), 0, "«до» раньше «с» — интервал пустой");
    eq(W.shiftHours([{ from: "ерунда", to: "18:00" }]), 0, "мусор во времени не считается");
    eq(W.shiftHours([{ from: "09:30", to: "12:00" }]), 2.5, "половинки часов считаются");
    const c = W.setCrew(2, [{ from: "10:00", to: "13:00" }, { from: "15:00", to: "18:00" }]);
    eq(c.hoursPerDay, 6, "setCrew возвращает посчитанные часы дня");
    eq(W.getCrew().hoursPerDay, 6, "и они переживают перечитывание (localStorage)");
    eq(W.getCrew().shift.length, 2, "оба захода сохранены");
    // старый формат {people, hoursPerDay} без расписания продолжает читаться — у кого он
    // сохранён, срок не поедет
    eq(W.crewHours({ people: 2, hoursPerDay: 8 }), 8, "явные часы старого формата главнее");
    const br = W.breakdown(WORKS, { crew: c });
    eq(br.hoursPerDay, 6, "разбивка знает часы дня");
    eq(br.shiftText, "10:00–13:00, 15:00–18:00", "и режим дня для документа");
    eq(br.days, Math.ceil((br.hours / 12) * 2) / 2, "срок считается от 2 чел. × 6 ч, а не × 8");
    ok(br.stages.every((s) => s.days > 0), "у КАЖДОГО этапа свои дни, не только общий итог");
    ok(br.stages.reduce((a, s) => a + s.days, 0) >= br.days,
      "общий срок считается от общих часов, а не суммой этапных (иначе округление копилось бы)");
    const html = loadEstimate().EstimatePrint.worksStagesHtml({ works: WORKS, crew: c });
    ok(/Режим работы/.test(html) && /10:00–13:00, 15:00–18:00/.test(html), "режим дня в печатном бланке");
    ok(/2 чел\. × 6 ч\/день/.test(html), "и бригада с реальными часами дня");
    ok(/раб\. дн\./.test(html.split("Итого по этапу")[1] || ""), "дни печатаются и у этапа");
  });
