...
(() => {
  ...
  // Нормализация — из ОБЩЕГО модуля EP.NameMatch (name-match.js), тем же пользуется
  // priceFor() в «Расчёте» проекта квартиры. Раньше у каждого была своя копия, и они
  // разошлись; фолбэк ниже — на случай, если модуль не подключён на странице.
  function cardNorm(s) {
    if (window.EP && window.EP.NameMatch) return window.EP.NameMatch.norm(s);
