...
(() => {
  ...
  function newPanel(x, y, name) { return { id: uid("pn"), x: x || 0, y: y || 0, name: name || "Щит", transformer: false, router: false, avr: false, neptun: false /* контроллер «Нептун»: цель датчиков протечки */ /* система АВР: автоматический ввод резерва (второй ввод/генератор) */, height: null /* null = settings.panelHeight (общая на проект) */, floorId: curFloorId() }; }
    ...
    if (!p || cloudAt > (p.updatedAt || 0)) {
      ...
      if (cp && (!p || (cp.updatedAt || 0) > (p.updatedAt || 0))) {
        ...
        if (p && (p.updatedAt || 0) > synced) {
          ...
          // conflictAt — до какой облачной версии копия УЖЕ сделана: без него каждое
          // открытие проекта плодило бы ещё одну копию (облако так и остаётся новее).
          if ((p.conflictAt || 0) < (cp.updatedAt || 0)) {
            saveConflictCopy(cp);
            p.conflictAt = cp.updatedAt;
