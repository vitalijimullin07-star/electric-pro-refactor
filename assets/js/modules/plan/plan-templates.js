...
(() => {
  ...
  const T = {
    ...
    applied: (n) => `Добавлено комнат: ${n}`,
    sizesHint: "Впиши свои размеры по обмеру — комнаты вставятся уже состыкованными, общие стены получатся сами.",
    sizesNote: "Ш — вдоль ряда, Г — глубина ряда. Потом всё правится как у обычной нарисованной комнаты."
  };
  ...
  const CATEGORIES = [
    ...
    { id: "r2s", name: "2-комнатная (раздельная)", area: "~50 м²", rows: ([
    ...
    { id: "r3", name: "3-комнатная", area: "~68 м²", rows: ([
  ...
  const SERIES = [
    ...
    { id: "p44-2", name: "П-44, 2-комн.", area: "~53 м²", rows: ([
    ...
    { id: "p44-3", name: "П-44, 3-комн.", area: "~72 м²", rows: ([
    ...
    { id: "i155-2", name: "И-155, 2-комн.", area: "~58 м²", rows: ([
    ...
    { id: "kope-2", name: "КОПЭ, 2-комн.", area: "~54 м²", rows: ([
  ...
  // rowsOverride — размеры, поправленные пользователем на экране «Размеры комнат»
  // (шаблон даёт КАРКАС и правильную стыковку, числа подставляет мастер по обмеру).
  function apply(tplId, rowsOverride) {
    const tpl = findTpl(tplId);
    const c = core(), p = c && c.project;
    if ((!tpl && !rowsOverride) || !p) return 0;
    const built = layoutRows(rowsOverride || tpl.rows);
  ...
  /* ---------- экран «Размеры комнат» ----------
     ...
     приближённые, под реальный объект всё равно перерисовывать). Теперь между выбором
     шаблона и вставкой — таблица размеров: шаблон даёт КАРКАС со стыковкой комнат
     (общие стены получаются по построению, разойтись не могут), а числа мастер вбивает
     свои, по обмеру. Поэтому «приближённость» раскладки перестала быть проблемой и
     шаблоны открыты всем. */
  let draftRows = null, draftTpl = null;
    ...
    return `<div class="ep-plan-srow"><b>${esc(tpl.name)}</b><span class="ep-plan-flex"></span>
      ...
      ${rows.map((row, ri) => `<div class="ep-plan-tplrow">
        ${row.map((r, ci) => `<div class="ep-plan-tplcell">
          <input type="text" value="${esc(r.name)}" maxlength="40" data-tpl-name="${ri}.${ci}">
          <label>Ш<input type="number" inputmode="numeric" min="30" max="3000" value="${Math.round(r.w)}" data-tpl-w="${ri}.${ci}"></label>
          <label>Г<input type="number" inputmode="numeric" min="30" max="3000" value="${Math.round(r.h)}" data-tpl-h="${ri}.${ci}"></label>
        </div>`).join("")}
      </div>`).join("")}
      <div class="ep-plan-srow"><b>Итого ≈ ${area.toFixed(1)} м²</b></div>
      <div class="ep-plan-modehint">${T.sizesNote}</div>
      <div class="ep-plan-srow ep-plan-sbtns">
        <button type="button" class="ep-plan-tbtn ep-clickable" data-plan-tpl-insert>✓ Вставить</button>
      </div>`;
  }
  function sheetSizes(tplId) {
    const tpl = findTpl(tplId);
    if (!tpl || !rooms()) return;
    draftTpl = tpl;
    // глубокая копия: правки размеров НЕ должны менять сам шаблон на следующий заход
    draftRows = tpl.rows.map((row) => row.map((r) => ({ name: r.name, w: r.w, h: r.h })));
    rooms().openSheet(sizesHtml(tpl, draftRows));
  }
  ...
  document.addEventListener("input", (e) => {
    const t = e.target;
    if (!t || !t.getAttribute || !draftRows) return;
    const at = (attr) => { const v = t.getAttribute(attr); if (!v) return null; const [ri, ci] = v.split("."); const r = draftRows[ri] && draftRows[ri][ci]; return r || null; };
    let r;
    if ((r = at("data-tpl-name"))) { r.name = t.value; return; }
    if ((r = at("data-tpl-w"))) { r.w = Math.max(30, Math.min(3000, Number(t.value) || 0)) || r.w; return; }
    if ((r = at("data-tpl-h"))) { r.h = Math.max(30, Math.min(3000, Number(t.value) || 0)) || r.h; return; }
  });
  ...
  document.addEventListener("click", (e) => {
    ...
    if (e.target.closest("[data-plan-tpl-back]")) return sheetPicker();
    if (e.target.closest("[data-plan-tpl-insert]")) {
    ...
    if (btn) return sheetSizes(btn.getAttribute("data-plan-tpl"));
  });
